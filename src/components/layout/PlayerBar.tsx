  ...
  return (
    <div className="w-full space-y-2">
      <div className="flex items-center gap-3">
        <span className="text-xs text-white/50 tabular-nums w-10 text-right font-light">
          {formatTime(displayProgress)}
        </span>
        <div
          ref={barRef}
          className="flex-1 relative h-6 flex items-center cursor-pointer touch-none"
        ...
        >
          ...
          {isBuffering ? (
            <motion.div
              className="absolute left-0 h-[2px] bg-white/40 rounded-full"
            ...
            <div
              ...
              style={{ width: `${percent}%`, transition: dragging ? 'none' : 'width 0.15s linear' }}
            />
          )}
        ...
        <span className="text-xs text-white/50 tabular-nums w-10 font-light">
          {formatTime(duration)}
        </span>
      </div>
    </div>
...
const CompactProgress = memo(({ progress, duration, isBuffering }: {
  ...
  return (
    <div className="w-full h-[2px] bg-white/10 overflow-hidden">
      {isBuffering ? (
        <motion.div className="h-full bg-white/40" animate={{ x: ['-100%', '100%'] }}
          transition={{ repeat: Infinity, duration: 1.2, ease: 'easeInOut' }} style={{ width: '40%' }} />
      ) : (
        <div className="h-full bg-white" style={{ width: `${percent}%`, transition: 'width 0.2s linear' }} />
      )}
    </div>
  );
});
...
const PlayPauseBtn = memo(({ size = 'md', isPlaying, isBuffering, playbackError, onPlay, onPause, onRetry }: {
  ...
  if (playbackError) {
    return (
      <motion.button whileTap={{ scale: 0.88 }} onClick={onRetry}
        className={cn(dims, 'rounded-full bg-white/10 border border-white/20 flex items-center justify-center')} title={playbackError}>
        <RotateCcw className={cn(iconSize, 'text-white')} />
      </motion.button>
    );
  }
  if (isBuffering) {
    return (
      <div className={cn(dims, 'rounded-full border-2 border-white/30 flex items-center justify-center')}>
        <Loader2 className={cn(iconSize, 'text-white animate-spin')} />
      </div>
    );
  }
  return (
    <motion.button whileTap={{ scale: 0.88 }} onClick={isPlaying ? onPause : onPlay}
      className={cn(dims, 'rounded-full border-2 border-white/80 flex items-center justify-center transition-colors hover:bg-white/10')}>
      {isPlaying
...
const CtrlBtn = memo(({ onClick, active, children, className }: {
  ...
  <button onClick={onClick}
    className={cn('flex items-center justify-center transition-all duration-200 active:scale-90',
      active ? 'text-white' : 'text-white/40 hover:text-white/70', className)}>
    {children}
  </button>
));
...
export const PlayerBar = () => {
  ...
  // Dynamic gradient from dominant color
  const bgGradient = `
    radial-gradient(ellipse at 50% 0%, rgba(${dominantColor.r},${dominantColor.g},${dominantColor.b},0.35) 0%, transparent 60%),
    radial-gradient(ellipse at 50% 100%, rgba(${dominantColor.r * 0.3},${dominantColor.g * 0.3},${dominantColor.b * 0.3},0.4) 0%, transparent 50%),
    linear-gradient(180deg, #0a0a0c 0%, #050506 100%)
  `;

  const handleArtworkSwipe = (_: any, info: PanInfo) => {
    if (Math.abs(info.offset.x) > 80 && Math.abs(info.velocity.x) > 100) {
      info.offset.x > 0 ? player.previous() : player.next();
    }
  };

  const handleVerticalGesture = (_: any, info: PanInfo) => {
    if (info.offset.y < -60) setQueueOpen(true);
  ...
  const handleDoubleTap = () => {
    ...
    if (now - lastTapRef.current < 300 && user) {
      ...
      if (!liked) { setShowHeartBurst(true); setTimeout(() => setShowHeartBurst(false), 800); }
    }
    ...
    return (
      <>
        ...
        <AnimatePresence>
          {queueOpen && (
            <motion.div
              ...
              dragElastic={0.3}
              onDragEnd={(_, info) => { if (info.offset.y > 100) setQueueOpen(false); }}
              className="fixed inset-0 z-[60] bg-black/95 backdrop-blur-xl rounded-t-3xl pt-3"
              style={{ top: 0 }}
            >
              ...
              <div className="px-5 pb-3 flex items-center justify-between">
                ...
                <button onClick={() => setQueueOpen(false)} className="text-white/40 text-xs">Done</button>
              </div>
              <div className="px-5 overflow-y-auto" style={{ maxHeight: '50vh' }}>
                {queue.slice(queueIndex + 1).map((track, i) => (
                  <button key={`${track.id}-${i}`} onClick={() => player.play(track, queue)}
                    ...
                    <img src={track.album_image} alt="" className="w-10 h-10 rounded-lg object-cover" loading="lazy" />
                ...
                {upcomingCount === 0 && <p className="text-center text-white/30 text-sm py-8">Queue is empty</p>}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          ...
          {/* Top bar */}
          <div className="flex items-center justify-between px-5 pt-[max(env(safe-area-inset-top),12px)] pb-1 relative z-10">
            <button onClick={() => setExpanded(false)} className="p-2 -ml-2 text-white/60 active:scale-90 transition-transform">
              <ChevronDown className="h-6 w-6" />
            </button>
            <div className="text-center">
              <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-white/40">Now Playing</p>
            </div>
            <button onClick={() => setQueueOpen(true)} className="p-2 -mr-2 text-white/60 active:scale-90 transition-transform relative">
          ...
          <div className="flex-1 flex items-center justify-center px-8 relative z-10">
            ...
            >
              ...
              <AnimatePresence>
                {showHeartBurst && (
                  <motion.div initial={{ scale: 0, opacity: 1 }} animate={{ scale: 1.6, opacity: 0 }}
              ...
              {user && (
                ...
                >
                  <Heart className={cn('h-6 w-6 drop-shadow-lg', liked ? 'text-white fill-current' : 'text-white/60')} />
                </motion.button>
              )}
            </motion.div>
          </div>
          ...
          {/* Track info */}
          <div className="px-8 relative z-10 space-y-5 pb-3">
            <AnimatePresence mode="wait">
              <motion.div
              ...
              >
                <p className="text-lg font-semibold text-white truncate">{currentTrack.name}</p>
                <p className="text-sm text-white/50 truncate mt-0.5">{currentTrack.artist_name}</p>
              </motion.div>
            </AnimatePresence>

            {/* Error */}
            {playbackError && <p className="text-xs text-red-400 text-center">{playbackError}</p>}
            ...
            <div className="flex items-center justify-between px-4">
              ...
              <CtrlBtn onClick={player.previous} className="text-white/70 hover:text-white">
              ...
              <CtrlBtn onClick={player.next} className="text-white/70 hover:text-white">
            ...
            <div className="flex items-center gap-3 px-2">
              <VolumeX className="h-3.5 w-3.5 text-white/25 flex-shrink-0" />
              <Slider value={[volume]} max={1} step={0.01}
                onValueChange={([v]) => player.setVolume(v)} className="flex-1" />
              <Volume2 className="h-3.5 w-3.5 text-white/25 flex-shrink-0" />
            </div>
    ...
    <>
      ...
      >
        <div className="absolute inset-0"
          style={{
            background: `linear-gradient(180deg, rgba(${dominantColor.r},${dominantColor.g},${dominantColor.b},0.15) 0%, hsl(0 0% 3% / 0.98) 100%)`,
        ...
        >
          <button onClick={() => isMobile && setExpanded(true)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
            <motion.div
              ...
              className={cn('rounded-lg overflow-hidden flex-shrink-0 shadow-lg', isMobile ? 'h-10 w-10' : 'h-12 w-12')}
            >
              {currentTrack.album_image ? (
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-medium truncate text-white">{currentTrack.name}</p>
              <p className="text-[11px] truncate text-white/40 mt-0.5">{currentTrack.artist_name}</p>
            </div>
          </button>

          {!isMobile && user && (
            <motion.button whileTap={{ scale: 0.85 }} onClick={handleLike}
              className={cn('p-1.5 rounded-full transition-all', liked ? 'text-white' : 'text-white/30 hover:text-white')}>
          ...
          <div className={cn('flex flex-col items-center', isMobile ? 'gap-0' : 'gap-1 flex-1')}>
            <div className="flex items-center gap-2">
              ...
              <CtrlBtn onClick={player.previous} className="p-1.5 text-white/60 hover:text-white"><SkipBack className="h-[18px] w-[18px] fill-current" /></CtrlBtn>
              <PlayPauseBtn size="sm" isPlaying={isPlaying} isBuffering={isBuffering}
                playbackError={playbackError} onPlay={player.resume} onPause={player.pause} onRetry={player.retryPlayback} />
              <CtrlBtn onClick={player.next} className="p-1.5 text-white/60 hover:text-white"><SkipForward className="h-[18px] w-[18px] fill-current" /></CtrlBtn>
          ...
          {!isMobile && (
            <div className="flex items-center gap-1 flex-1 justify-end">
              {sleepTimer !== null && (
                <span className="text-[10px] text-white/40 flex items-center gap-0.5 mr-1"><Timer className="h-3 w-3" />{sleepTimer}m</span>
              )}
              <button onClick={() => setQueueOpen(o => !o)} className="p-1.5 text-white/30 hover:text-white transition-colors relative">
              ...
              <Slider value={[volume]} max={1} step={0.01}
                onValueChange={([v]) => player.setVolume(v)} className="w-20" />
            </div>
          )}
        </motion.div>
      </motion.div>
